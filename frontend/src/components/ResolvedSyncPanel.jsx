...
export function ResolvedSyncPanel() {
  ...
  const load = () => api.get("/admin/servicenow/sync").then(r => setCfg(r.data));
  useEffect(() => { load(); }, []);
  ...
  const save = async () => {
    try {
      ...
      setCfg(c => ({ ...c, ...data }));
      toast.success(data.enabled ? `Nightly sync enabled — runs at ${String(data.hour_utc).padStart(2, "0")}:00 UTC with ${data.sync_user_email}` : "Nightly sync disabled");
    } catch (e) { if (e?.response?.status !== 428) toast.error(e?.response?.data?.detail || "Save failed"); else toast.error(e.response.data.detail); }
  };
  ...
  const runNow = async () => {
    ...
    try {
      ...
      data.ok ? toast.success(`Synced ${data.fetched} resolved incidents — ${data.inserted} new, ${data.updated} updated`) : toast.error(data.error || "Sync failed", { duration: 8000 });
      load();
    } catch (e) { if (e?.response?.status !== 428) toast.error(e?.response?.data?.detail || "Sync failed"); }
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {last && (
          <div className="text-xs text-slate-400 flex items-center gap-2 flex-wrap" data-testid="sync-last-result">
            <Badge className={`border ${last.ok ? "bg-emerald-500/15 text-emerald-300 border-emerald-500/40" : "bg-rose-500/15 text-rose-300 border-rose-500/40"}`}>{last.ok ? "OK" : "Failed"}</Badge>
