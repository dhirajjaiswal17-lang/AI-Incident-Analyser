...
- 2026-09-06: v1.6 — Three enhancements. (1) **Switched AI to Universal Key**: AI config now uses Emergent Universal Key (openai/gpt-5.4, use_emergent_key=true) instead of owner's Gemini free-tier BYOK — no more daily quota 424s; `_run_llm` forces temperature=1 for gpt-5* models (gpt-5 only supports temp=1). AI test + manual analysis verified working end-to-end with real output that cites pasted logs. (2) **Copy analysis** button on `AnalysisResult` (data-testid copy-analysis-btn) — copies formatted RCA/impact/actions/confidence to clipboard. (3) **One-Click Apply Fix**: new `POST /api/incidents/{sys_id}/apply-fix` (ApplyFixRequest{historical_id}) posts a matched historical incident's proven resolution as a ServiceNow work note; button added to each Similar-Incidents historical row (data-testid apply-fix-btn-{id}), disabled in demo, shows "posted" state. Endpoint backend-verified (404 bad id, SN-credential guard); full live UI run needs real SN creds.

- 2026-09-06: v1.7 — **Auto-Analyze New P1s**. Background poller (`_auto_analyze_scheduler`, checks every minute, runs when poll_minutes elapsed) fetches active priority-1 incidents from ServiceNow and runs the AI analysis automatically so a root-cause draft awaits the on-call. Config kind="auto_analyze" {enabled(false), poll_minutes(15,5-180), max_per_cycle(10,1-50)}; uses the Resolved-Sync service account (sync_user_id) + current AI config; de-dups by incident_sys_id (won't re-analyze). AnalysisRecord gained `auto` flag. New endpoints: GET/PUT/POST-run `/api/admin/auto-analyze`, `GET /api/incidents/{sys_id}/auto-analysis` (on-call retrieval). Feedback endpoint relaxed to allow any user on auto records. UI: `AutoAnalyzePanel` on Admin → ServiceNow (toggle, interval, cap, Run now, last-result badge); IncidentDetail shows the ready auto-analysis with an amber "Auto-analyzed for the on-call" banner (via `useAutoAnalysis`), re-runnable. Verified end-to-end on live ServiceNow: real P1 fetched → analyzed with openai/gpt-5.4 → retrievable → 2nd run de-dups (analyzed 0). Default left DISABLED (avoids silent credit spend until admin enables).
